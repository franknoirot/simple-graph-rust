...
const WITH_FOREIGN_KEY_PRAGMA: &str = "%s?_foreign_keys=true";
const ID_CONSTRAINT: &str = "NOT NULL constraint failed: nodes.id";
const UNIQUE_ID_CONSTRAINT: &str = "UNIQUE constraint failed: nodes.id";
const NO_ROWS_FOUND: &str = "sql: no rows in result set";

struct NodeData {
    body: Option<&'static str>,
}
impl NodeData {
    const ID: &'static str = "json:\"id\"";
}
...
// The rest still hasn't been converted from Go 
